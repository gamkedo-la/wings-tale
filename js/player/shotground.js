...
function shotGroundClass(
  ...
  this.move = function () {
    if (this.homingTarget != null) {
      if (this.homingTarget.readyToRemove) {
        ...
        var angAt = Math.atan2(
          this.homingTarget.y - startY,
          this.homingTarget.x - startX
        );
        if (
          approxDist(this.x, this.y, this.homingTarget.x, this.homingTarget.y) <
          BOMB_RADIUS
        ) {
          // near, drop faster
    ...
    if (this.y < 0 || this.x < 0 || this.x > GAME_W || this.y > GAME_H) {
      ...
      for (var i = 0; i < BOMB_EXPLOSIONS; i++) {
        ...
        spawnSplode(
          this.x + Math.cos(blastAng) * blastDist,
          this.y + Math.sin(blastAng) * blastDist,
          true
        );
      }
      listCollideRangeOfPoint(
        surfaceList,
        this.x,
        this.y,
        BOMB_RADIUS,
        function (at) {
          if (Math.random() < GROUND_POWERUP_DROP_PERCENT) {
            powerupList.push(new powerupClass(at.x, at.y, [0, 1, 4]));
          }
        }
      );
      this.readyToRemove = true;
  ...
  this.draw = function () {
    var frameNow =
      PLAYER_SHOT_GROUND_FRAMES -
      1 -
      Math.floor((-this.yv / SHOT_GROUND_SPEED) * PLAYER_SHOT_GROUND_FRAMES);
    drawAnimFrame(
      "player shot ground",
      this.x,
      this.y,
